...
class unlock extends action{
    ...
    public function _execute($args){
        if (count($args)==0){
            ...
            $result['data']="What do you want to unlock?";
        }
        $can = $this->can($args);
        if ($can->result==true){
            $this->unlock_it($can->item);

            $action_responses = json_decode($can->item->action_responses);

            $response = $action_responses->unlock;
            $result['action']="OUTPUT";
            ...
            $this->register_event('unlock',$user_id,$user_loc);            
        }
        else{
            $result = array();
            $result['action']="OUTPUT";
    ...
    public function can($args){
        ...
        if (!($item_type = $this->is_item($words))){
            ...
            if (array_search("unlock",$actions)===FALSE){
                $return->result=false;
                ...
                if (!($items=$this->is_item_in_location($item_type))){
                    ...
                    if (count($items)>1){
                        ...
                        if (!$this->is_it_locked($item)){
                            $return->result=false;
                            $return->reason="That is already unlocked";
                        }
    ...
    private function unlock_it($item){
        $attributes = json_decode($item->attributes);
